...
public class DepartmentProfileListView extends VerticalLayout {
    ...
    public DepartmentProfileListView(DepartmentProfileService departmentProfileService, UserService userService) {
    ...
    public HorizontalLayout buildRowToolbar() {
        ...
        deleteButton.setTooltipText("Delete Assigned Department");
        deleteButton.setIcon(LineAwesomeIcon.TRASH_ALT_SOLID.create());
        deleteButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY, ButtonVariant.LUMO_ERROR);
        deleteButton.addClickListener(buttonClickEvent -> {
            if (employeeDepartmentDTOGrid.getSelectionModel().getFirstSelectedItem().isPresent()) {
                DepartmentProfileDTO selectedDepartmentProfileDTO = employeeDepartmentDTOGrid.getSelectionModel()
                        .getFirstSelectedItem().get();

                // Check first if that record is not an active employee anymore. If it is not active, you may proceed
                // for the deletion.
                if (selectedDepartmentProfileDTO.getEmployeeDTO().getStatus().equals("RESIGNED")
                        || selectedDepartmentProfileDTO.getEmployeeDTO().getStatus().equals("RETIRED")
                        || selectedDepartmentProfileDTO.getEmployeeDTO().getStatus().equals("TERMINATED")
                        || selectedDepartmentProfileDTO.getEmployeeDTO().getStatus().equals("DECEASED")) {
                    // Show the confirmation dialog.
                    ConfirmDialog confirmDialog = new ConfirmDialog();
                    confirmDialog.setHeader("Delete Assigned Department");
                    ...
                    confirmDialog.addConfirmListener(confirmEvent -> {
                        ...
                        departmentProfileService.delete(selectedDepartmentProfileDTO);

                        // Refresh the data grid from the backend after the delete operation.
                        employeeDepartmentDTOGrid.getDataProvider().refreshAll();

                        // Show notification message.
                        Notification notification = Notification.show("You have successfully deleted the selected employee in the assigned department.",
                                5000,
                    ...
                    // Show notification message.
                    Notification notification = Notification.show("You cannot delete the selected employee in the assigned department. Employee is still in active status.",
                            5000,
                            Notification.Position.TOP_CENTER);
